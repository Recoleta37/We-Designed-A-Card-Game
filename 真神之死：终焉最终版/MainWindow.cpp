...
MainWindow::MainWindow(QWidget* parent) : QWidget(parent), storyManager(this), mapManager(this), battleEngine(this, &storyManager, &mapManager)
{
    ...
    battleEngine.setReturnTitleCallback([this]() { showTitleScreen(); });
    initData();
...
void MainWindow::updateOverlayGeometry()
{
    ...
    updateTitleOverlayGeometry();
}

void MainWindow::buildTitleOverlay()
{
    titleOverlay = new QWidget(this);
    titleOverlay->setObjectName(QStringLiteral("titleOverlay"));
    titleOverlay->hide();

    titleBackgroundLabel = new QLabel(titleOverlay);
    titleBackgroundLabel->setAlignment(Qt::AlignCenter);
    titleBackgroundLabel->setScaledContents(false);

    const QString titlePath = styleAssetUrl(QStringLiteral("assets/story/prologue/title.png"));
    if (!titlePath.isEmpty()) titleBackgroundPixmap.load(titlePath);

    titleMainLabel = new QLabel(QStringLiteral("真神之死：终焉"), titleOverlay);
    titleMainLabel->setAlignment(Qt::AlignLeft | Qt::AlignVCenter);
    titleMainLabel->setStyleSheet(QStringLiteral(
        "background:transparent; color:#f8e9bd; font-size:72px; font-weight:900;"
        "text-shadow:none;"));
    ...
    for (int i = 0; i < int(titleKeywordLabels.size()); ++i)
    {
        titleKeywordLabels[i] = new QLabel(keywords[i], titleOverlay);
        titleKeywordLabels[i]->setAlignment(Qt::AlignLeft | Qt::AlignVCenter);
        titleKeywordLabels[i]->setStyleSheet(QStringLiteral(
            "background:transparent; color:#ead59d; font-size:28px; font-weight:700;"));
    }

    titleQuestionLabel = new QLabel(QStringLiteral("以及一个问题"), titleOverlay);
    titleQuestionLabel->setAlignment(Qt::AlignLeft | Qt::AlignVCenter);
    titleQuestionLabel->setStyleSheet(QStringLiteral(
        "background:transparent; color:#f2dfad; font-size:30px; font-weight:700;"));

    titleStartButton = new QPushButton(QStringLiteral("开始游戏"), titleOverlay);
    titleStartButton->setStyleSheet(QStringLiteral(
        "QPushButton { background:rgba(72,38,15,220); color:#ffe8b3; border:2px solid #d6ad5e;"
        "border-radius:7px; padding:12px 30px; font-size:22px; font-weight:800; }"
        "QPushButton:hover { background:rgba(112,68,30,235); }"
        "QPushButton:pressed { background:rgba(55,28,12,240); }"));
    connect(titleStartButton, &QPushButton::clicked, this, [this]() {
        titleOverlay->hide();
        startGame();
    });
}
...
void MainWindow::showTitleScreen()
{
    ...
    widgets << titleMainLabel;
    for (QLabel* label : titleKeywordLabels) widgets << label;
    widgets << titleQuestionLabel;
    widgets << titleStartButton;
    ...
    QTimer::singleShot(350, this, [this]() { fadeInTitleWidget(titleMainLabel, 1200); });
    for (int i = 0; i < int(titleKeywordLabels.size()); ++i)
    {
        QTimer::singleShot(1450 + i * 520, this, [this, i]() {
            fadeInTitleWidget(titleKeywordLabels[i], 650);
        });
    }
    QTimer::singleShot(3600, this, [this]() { fadeInTitleWidget(titleQuestionLabel, 760); });
    QTimer::singleShot(4450, this, [this]() { fadeInTitleWidget(titleStartButton, 900); });
}

void MainWindow::updateTitleOverlayGeometry()
{
    if (!titleOverlay) return;
    const QRect fullRect(0, 0, width(), height());
    titleOverlay->setGeometry(fullRect);
    if (titleBackgroundLabel)
    {
        titleBackgroundLabel->setGeometry(fullRect);
        if (!titleBackgroundPixmap.isNull())
        {
            QPixmap scaled = titleBackgroundPixmap.scaled(fullRect.size(), Qt::KeepAspectRatioByExpanding, Qt::SmoothTransformation);
            titleBackgroundLabel->setPixmap(scaled);
        }
    }

    const int left = std::max(48, width() / 12);
    const int top = std::max(88, height() / 7);
    if (titleMainLabel)
        titleMainLabel->setGeometry(left, top, std::min(width() - left - 40, 900), 96);

    const int keywordTop = top + 118;
    const int keywordWidth = 80;
    const int keywordHeight = 48;
    const int gap = 22;
    for (int i = 0; i < int(titleKeywordLabels.size()); ++i)
    {
        if (titleKeywordLabels[i])
            titleKeywordLabels[i]->setGeometry(left + i * (keywordWidth + gap), keywordTop, keywordWidth, keywordHeight);
    }

    if (titleQuestionLabel && titleKeywordLabels[0])
        titleQuestionLabel->setGeometry(titleKeywordLabels[0]->x(), keywordTop + 58, 320, 48);

    if (titleStartButton)
    {
        const QSize buttonSize(190, 62);
        titleStartButton->setGeometry((width() - buttonSize.width()) / 2,
                                      height() - std::max(130, height() / 6),
                                      buttonSize.width(),
                                      buttonSize.height());
    }
}

void MainWindow::fadeInTitleWidget(QWidget* widget, int durationMs)
{
    if (!widget) return;
